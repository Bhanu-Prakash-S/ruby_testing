...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      subject(:monday) {described_class.new("slice")}
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(monday.type).to eq "slice"
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(monday.ounces).to eq 16
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:tuesday) {described_class.new("maaza", 20)}
      # remove the 'x' before running this test
      it 'is full' do
        expect(tuesday.full?).to be true
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:wed) {described_class.new("sprite", 2)}

      # remove the 'x' before running this test
      it 'is not full' do
        expect(wed.full?).to be false
      end
    end
  end
end
