...
describe FindNumber do
  ...
  context 'when min and max both equal 3' do
    let(:number_guessing) { double('random_number', :value => 8) }
    subject(:game_guessing) { described_class.new(0, 6, number_guessing) }
    it 'returns 3' do
        solution = game_guessing.make_guess
        expect(solution).to eql 3      
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:random_num) { double('random_number', :value => 5) }
      subject(:game) { described_class.new(0, 9, random_num, 5) }
    ...
    context 'when guess and random_number are not equal' do
      let(:random_num) { double('random_number', :value => 4) }
      subject(:game) { described_class.new(0, 9, random_num, 5) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      let(:game_7) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        expect { game_7.update_range }.to change { game_7.min }.from(game_7.min).to(game_7.max)
      end

      it 'does not update max' do
        expect { game_7.update_range }.to_not change { game_7.max }
      end
    end
  end
end
